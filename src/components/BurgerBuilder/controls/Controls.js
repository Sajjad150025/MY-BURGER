...
const BuildControl = props =>{
    return(
        <div className='d-flex'>
            <div className='me-auto ml-5' style={{fontSize: '1.2rem', fontWeight: 'bold'}}>{props.label}</div>
  ...
  return (
    <div className='container ml-md-5' style={{textAlign: 'center'}}>
        <Card style={{marginTop: '30px', marginBottom: '30px', textAlign: 'center'}}>
            ...
            <CardBody>
                {controls.map(item =>{ return <BuildControl label={item.label} type={item.type} key={Math.random()} added={() => props.ingredientAdded(item.type)} removed={()=> props.ingredientRemove(item.type)} />})}
            </CardBody>
